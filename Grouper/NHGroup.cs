 ...
 * File: Sketch.cs
 *
 * Authors: Originally by Unknown, Modified by James Brown
...
namespace Grouper
{
	...
	/// </summary>
	internal class NHGroup : Group, IGroup
	{
        // Jason's addition
        private const ulong THRESHOLD = 2000;
		

		public NHGroup(Sketch.Sketch sketch) : base(sketch)
		{
        ...
        /// <summary>
        /// Group the substrokes using a naive grouping algorithm
        /// </summary>
        /// <param name="verbose">Debug if true</param>
        public override void group(bool be_verbose)
        {
			verbose = be_verbose;
            ...
            foreach(LabelGroup lg in labelGroups)
            {
                hierarchicalCluster(lg);
            }
        }
      
		/// <summary>
		/// Clusters a list of substrokes into small groups
		/// </summary>
		/// <param name="substrokes">The strokes to cluster</param>
		/// <param name="label">The label to apply to the cluster</param>
		public void hierarchicalCluster(List<Substroke> substrokes, string label)
		{
			LabelGroup lg = new LabelGroup(label);
			foreach (Substroke ss in substrokes)
				lg.addSubstroke(ss);
			hierarchicalCluster(lg);
		}
